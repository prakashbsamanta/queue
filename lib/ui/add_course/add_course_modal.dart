  ...
  void _submit() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_addToExisting) {
        ...
        await ref
            .read(addCourseLogicProvider.notifier)
            .addToExistingCourse(_selectedCourseId!, input, type);
      } else {
        // Create New
        if (isYouTube) {
          await ref
              .read(addCourseLogicProvider.notifier)
              .addCourseFromUrl(input);
        } else if (input.startsWith('http')) {
          // It's a URL but not YouTube - create course and add as URL resource
          // First create an empty course with a meaningful name
          final courseName = input.contains('://')
              ? Uri.parse(input).host.replaceAll('www.', '')
              : 'Article';
          await ref
              .read(addCourseLogicProvider.notifier)
              .addCourseByName(courseName);

          // Get the newly created course ID (last added course)
          final courses = await ref.read(allCoursesProvider.future);
          if (courses.isNotEmpty) {
            final newCourse = courses.first; // Most recent
            await ref
                .read(addCourseLogicProvider.notifier)
                .addToExistingCourse(newCourse.id, input, 'url');
          }
        } else {
          // Just text - create empty course with the name
          await ref
              .read(addCourseLogicProvider.notifier)
              .addCourseByName(input);
        }
      }
      ...
      if (mounted && !ref.read(addCourseLogicProvider).hasError) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content:
                  Text(_addToExisting ? 'Resource Added' : 'Course Created')),
        );
      }
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              NeoTextField(
                ...
                hintText: _addToExisting
                    ? 'Paste Link or type Text'
                    : 'Paste YouTube URL or Course Name',
